  ...
  return (
    <div className="space-y-6 text-[#FAF9F6] selection:bg-[#BFDD25] selection:text-black">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-3.5 py-1.5 bg-[#222] hover:bg-[#333] text-white font-mono font-bold rounded-full cursor-pointer transition-colors"
              >
      ...
      <div className="bg-[#0A0A0A] rounded-2xl overflow-hidden shadow-xl">
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-xl bg-[#141414] flex items-center justify-center font-mono font-bold text-xs text-white shrink-0">
                            {store.storeName.charAt(0)}
                          </div>
                          <div className="flex flex-col min-w-0">
                            <span
                              className="font-bold text-white hover:text-[#BFDD25] cursor-pointer truncate transition-colors"
                      ...
                      <td className="py-3.5 px-4">
                        <span className="px-2.5 py-0.5 rounded-full bg-[#141414] text-xs font-mono text-[#aaa]">
                          {store.brandFocus}
                        </span>
                      </td>
                      ...
                      <td className="py-3.5 px-4">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-mono font-medium bg-[#141414] text-[#D4D4D8]">
                          <span className={`w-1.5 h-1.5 rounded-full ${
                            store.status === "APPROVED" ? "bg-[#BFDD25] shadow-[0_0_6px_rgba(191,221,37,0.8)]" : store.status === "PENDING" ? "bg-amber-400" : "bg-rose-400"
                          }`} />
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {store.status === "APPROVED" && (
                            <button
                              onClick={() => handleBulkSuspend()}
                              className="px-3 py-1 bg-[#141414] hover:bg-rose-500/20 text-[#888] hover:text-rose-400 text-xs font-mono rounded-full transition-colors cursor-pointer"
                            >
                              {isEn ? "Suspend" : "Nonaktifkan"}
                            </button>
                          )}
        ...
        <div className="p-4 bg-[#0A0A0A] flex flex-col sm:flex-row items-center justify-between text-xs font-mono text-[#777] gap-2">
          <span>
            {isEn ? `Showing ${processedStores.length} of ${stores.length} stores` : `Menampilkan ${processedStores.length} dari ${stores.length} toko`}
          </span>
          <span className="text-[11px] text-[#555]">{isEn ? "Tonal Zone Merchant Verification" : "Verifikasi Toko Tonal Zone"}</span>
        </div>
      </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="bg-[#141414] p-3.5 rounded-xl space-y-1">
                  ...
                  <p className="text-[#BFDD25] text-[11px] font-mono">
              ...
              <div className="bg-[#141414] p-4 rounded-xl flex items-center justify-between">
                <div>
                  ...
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-mono font-medium bg-[#1c1c1c] text-[#D4D4D8]">
                    <span className={`w-1.5 h-1.5 rounded-full ${
                      viewingStore.status === "APPROVED" ? "bg-[#BFDD25] shadow-[0_0_6px_rgba(191,221,37,0.8)]" : viewingStore.status === "PENDING" ? "bg-amber-400" : "bg-rose-400"
                    }`} />
                    {viewingStore.status}
                  </span>
                </div>
