...
export async function POST(request: Request) {
  try {
    ...
    const newReturn = await returnRepo.create({
      ...
      productImage: firstItem?.image || "/model-iem-untuk-hero.webp",
      productPrice: firstItem?.price || order.totalAmount,
      quantity: firstItem?.quantity || 1,
      selectedVariant: firstItem?.selectedVariant || "Standard",
      reason,
      description,
      evidenceImages: Array.isArray(evidenceImages) && evidenceImages.length > 0 ? evidenceImages : ["/model-iem-untuk-hero.webp"],
