...
function Digit({ place, value, height, digitStyle }: DigitProps) {
  ...
  // Start spring at 0 so it rolls up to the target value on mount
  ...
  return (
    <span className="counter-digit" style={{ height, ...digitStyle }}>
      {Array.from({ length: 10 }, (_, i) => (
        <Number key={i} mv={animatedValue as unknown as MotionValue<number>} number={i} height={height} />
      ))}
    </span>
  );
}
...
export default function Counter({
  ...
  const defaultPlaces = places || [...value.toString()].map((ch, i, a) => {
    if (ch === '.') {
      return '.';
    } else {
      return (
