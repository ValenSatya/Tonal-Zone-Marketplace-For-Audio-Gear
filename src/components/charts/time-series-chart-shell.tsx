...
const TimeSeriesChartCore = memo(function TimeSeriesChartCore({
  ...
  const xAccessor = useCallback(
    (d: Record<string, unknown>, index?: number): Date => {
      ...
      // Safe fallback when data point lacks date field
      const fallbackTime = Date.now() - (10 - (index ?? 0)) * 86400000;
  ...
  const dateLabels = useMemo(
    () =>
      visiblePlotData.map((d, i) => {
        try {
          const date = xAccessor(d, i);
          return isNaN(date.getTime()) ? "" : shortDateFmt.format(date);
        } catch {
          return "";
        }
      }),
